...
function ModeratorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0a2610] text-white flex flex-col relative overflow-hidden font-sans selection:bg-white/30">
      {/* Cinematic Background */}
      <div className="fixed inset-0 bg-[#0a2610] z-0">
...
function CourseBuilder({ levelId, onBack }: { levelId: string | null; onBack: () => void }) {
  ...
  const addBlock = (type: "text" | "code" | "image" | "pdf" | "download" | "word" | "quiz" | "canvas", lectureIndex: number) => {
    ...
    const newBlock: ContentBlock = {
      ...
      content:
        type === "text"
          ? "Explain the concept, then give one practical instruction. Keep this step short and action-oriented."
          : type === "code"
            ? "// Starter code\n"
            : "",
    };
  ...
  const applyOpenCourseTemplate = () => {
    if (selectedLectureIdx === null || !lectures[selectedLectureIdx]) {
      toast.error(isAr ? "اختر محاضرة أولاً" : "Select a lecture first.");
      return;
    }

    const nl = [...lectures];
    const current = nl[selectedLectureIdx];
    current.title = current.title || "Build a Small Programming Exercise";
    current.description =
      current.description ||
      "A freeCodeCamp-style lesson: one clear objective, short explanation, starter code, immediate practice, and a check-for-understanding question.";
    current.content_blocks = [
      {
        id: uuidv4(),
        type: "text",
        content:
          "Objective: learn one concept by building something small. Read the explanation, change the starter code, then test the result before moving on.",
      },
      {
        id: uuidv4(),
        type: "text",
        content:
          "Instructions:\n1. Read the problem carefully.\n2. Predict what the code should output.\n3. Edit the starter code.\n4. Run it and compare your result.\n5. Write one sentence explaining why it works.",
      },
      {
        id: uuidv4(),
        type: "code",
        content:
          "// Starter code\nfunction solve(input) {\n  // Write your solution here\n  return input;\n}\n\nconsole.log(solve('CodeStart'));",
      },
      {
        id: uuidv4(),
        type: "quiz",
        ...
        metadata: {
          quiz: {
            ...
            options: [
              "Read the problem and identify the expected output",
              "Copy a random solution",
              "Skip testing",
              "Memorize the syntax only",
            ],
    ...
    setLectures(nl);
    toast.success(isAr ? "تم تطبيق قالب الدرس" : "Open-course lesson template applied");
  };

  const addChallengeCheckpoint = () => {
    if (selectedLectureIdx === null || !lectures[selectedLectureIdx]) return;

    const nl = [...lectures];
    if (!nl[selectedLectureIdx].content_blocks) nl[selectedLectureIdx].content_blocks = [];
    nl[selectedLectureIdx].content_blocks!.push(
      {
        id: uuidv4(),
        type: "text",
        content:
          "Checkpoint: explain the idea in your own words, then complete the task without looking at the solution.",
      },
      {
        id: uuidv4(),
        type: "code",
        content:
          "// Challenge checkpoint\n// 1. Read the expected result\n// 2. Write the missing logic\n// 3. Test with at least two inputs\n",
      },
      {
        id: uuidv4(),
        type: "quiz",
        ...
        metadata: {
          quiz: {
            question: "What proves you understood this lesson?",
            options: [
              "I can explain the idea and solve a similar task",
              "I watched the video only",
              "I copied the final code",
    ...
    setLectures(nl);
    toast.success(isAr ? "تمت إضافة نقطة تحقق" : "Challenge checkpoint added");
  };

  const moveBlock = (lectureIndex: number, fromIndex: number, toIndex: number) => {
    const nl = [...lectures];
    const blocks = nl[lectureIndex].content_blocks;
    if (!blocks || toIndex < 0 || toIndex >= blocks.length) return;
    const [movedBlock] = blocks.splice(fromIndex, 1);
    blocks.splice(toIndex, 0, movedBlock);
    setLectures(nl);
  };

  const duplicateBlock = (lectureIndex: number, blockIndex: number) => {
    const nl = [...lectures];
    const blocks = nl[lectureIndex].content_blocks;
    if (!blocks) return;
            ...
            {activeTab === "curriculum" &&
              ...
              lectures[selectedLectureIdx] && (
                <motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                  className="max-w-7xl space-y-10"
                >
                  <div className="flex justify-between items-end border-b border-white/5 pb-10">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                      <div className="rounded-3xl border border-blue-400/15 bg-blue-500/10 p-5">
                        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-blue-200">1. Teach</p>
                        <p className="mt-2 text-sm leading-6 text-white/60">One clear objective, one short explanation, one example students can run.</p>
                      </div>
                      <div className="rounded-3xl border border-emerald-400/15 bg-emerald-500/10 p-5">
                        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-emerald-200">2. Practice</p>
                        <p className="mt-2 text-sm leading-6 text-white/60">A starter code block and a task that changes something meaningful.</p>
                      </div>
                      <div className="rounded-3xl border border-amber-400/15 bg-amber-500/10 p-5">
                        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-amber-200">3. Verify</p>
                        <p className="mt-2 text-sm leading-6 text-white/60">A quiz or checkpoint before the next lesson unlocks.</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          <button
                            onClick={applyOpenCourseTemplate}
                            className="p-2 bg-blue-500/20 text-blue-200 border border-blue-400/20 rounded-lg hover:bg-blue-500 hover:text-white transition-colors text-[10px] font-black uppercase"
                          >
                            FCC Template
                          </button>
                          <button
                            onClick={addChallengeCheckpoint}
                            className="p-2 bg-emerald-500/20 text-emerald-200 border border-emerald-400/20 rounded-lg hover:bg-emerald-500 hover:text-white transition-colors text-[10px] font-black uppercase"
                          >
                            Checkpoint
                          </button>
                      ...
                      <div className="space-y-4">
                        <div className="rounded-3xl border border-blue-400/15 bg-blue-500/10 p-5">
                          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-blue-200">Editable seed content</p>
                          <p className="mt-2 text-sm leading-6 text-white/60">
                            Every seeded module loads here as editable content. You can rewrite the explanation, replace sandbox code, change project steps, and edit every MCQ directly from this panel.
                          </p>
                        </div>
                        {lectures[selectedLectureIdx]?.content_blocks?.map((block, bIdx) => {
                          ...
                          <div
                            key={block.id}
                            className={`relative group p-6 rounded-3xl border bg-white/[0.025] ${block.type === "code" ? "border-lime-400/20" : block.type === "quiz" ? "border-amber-400/20" : block.type === "text" ? "border-blue-400/15" : "border-white/10"}`}
                          >
                            <div className="mb-5 flex flex-wrap items-center justify-between gap-4 border-b border-white/5 pb-4">
                              <div className="flex items-center gap-3">
                                <span className="grid h-8 w-8 place-items-center rounded-xl bg-white/10 text-[10px] font-black text-white/60">
                                  {bIdx + 1}
                                </span>
                                <div>
                                  <span className="text-[9px] font-black uppercase tracking-[0.25em] text-[#CCFF00]">
                                    {block.type} block
                                  </span>
                                  <p className="mt-1 text-xs text-white/35">
                                    {block.type === "text" && "Editable teaching copy, project steps, hints, or sandbox instructions."}
                                    {block.type === "code" && "Editable starter code students can test in the sandbox."}
                                    {block.type === "quiz" && "Editable MCQ check for this module."}
                                    {!["text", "code", "quiz"].includes(block.type) && "Editable supporting asset for this lesson."}
                                  </p>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => moveBlock(selectedLectureIdx, bIdx, bIdx - 1)}
                                  disabled={bIdx === 0}
                                  className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/45 hover:text-white disabled:opacity-30"
                                >
                                  Up
                                </button>
                                <button
                                  onClick={() => moveBlock(selectedLectureIdx, bIdx, bIdx + 1)}
                                  disabled={bIdx === (lectures[selectedLectureIdx].content_blocks?.length || 0) - 1}
                                  className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/45 hover:text-white disabled:opacity-30"
                                >
                                  Down
                                </button>
                                <button
                                  onClick={() => duplicateBlock(selectedLectureIdx, bIdx)}
                                  className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase text-white/45 hover:text-white"
                                >
                                  Copy
                                </button>
                                <button
                                  ...
                                  className="rounded-xl bg-red-500/15 p-2 text-red-300 hover:bg-red-500 hover:text-white"
                                >
                                  <X className="w-4 h-4" />
                                </button>
                              </div>
                            ...
                            {block.type === "text" && (
                              <div className="space-y-2">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Editable Lesson Copy</p>
                                <textarea
                                  value={block.content || ""}
                                  onChange={(e) => {
                                    const nl = [...lectures];
                                  ...
                                  placeholder="Enter lesson explanation, instructions, sandbox steps, or project guidance..."
                                  className="w-full bg-black/40 p-4 rounded-xl outline-none text-sm leading-6"
                                  rows={6}
                                />
                              </div>
                            )}
                            ...
                            {block.type === "code" && (
                              <div className="space-y-2">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Editable Sandbox Code</p>
                                <textarea
                                  value={block.content || ""}
                                  onChange={(e) => {
                                    const nl = [...lectures];
                                  ...
                                  placeholder="Enter starter code, challenge code, or solution snippet..."
                            ...
                            {block.type === "quiz" && (
                              <div className="space-y-3">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30">Editable Module MCQ</p>
                                <input
                                  type="text"
